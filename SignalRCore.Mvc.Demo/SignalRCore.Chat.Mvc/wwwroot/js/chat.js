    ...
    if (connection === null) {
        ...
        connection.on("Broadcast", function (response) {
            $("#chatRoom").append(
                ...
                '<p class="text-small mb-0 text-white">' + response.body + '</p>' +
                '</div>' +
        ...
        connection.on("UserConnected", function (response) {
            ...
            response.forEach(user => {
                $("#chatUsers").append(
                    ...
                    '<h6 class="mb-0">' + user.userName + '</h6><small class="small font-weight-bold">' + user.connectionTime + '</small>' +
                    '</div>' +
                    '<p class="font-italic mb-0 text-small">' + user.connectionId + '</p>' +
                    '</div>' +
                    '</div>' +
                    '</a>'
                );
            ...
            $("#chatRoom").append(
                ...
                '</div>' +
                '<p class="small text-muted">' + response[response.length-1].messageDt + '</p>' +
                '</div>'
            );
        });
